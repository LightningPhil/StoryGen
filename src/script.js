 ...
 * the currently highlighted word.  Uses the DOM order of .story-word spans.
 */
function getReadAloudText() {
    if (!storyOutputDiv) return '';

    const words = storyOutputDiv.querySelectorAll('.story-word');
    if (words.length === 0) return storyOutputDiv.textContent || '';
    ...
    // Collect text from the start word onward, respecting paragraph breaks
    // by walking from the start word's parent node forward.
    const range = document.createRange();
    range.setStartBefore(words[startIdx]);
    range.setEndAfter(storyOutputDiv.lastChild);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    readAloudLabel = document.getElementById('readAloudLabel');
    stopReadAloudButton = document.getElementById('stopReadAloudButton');
