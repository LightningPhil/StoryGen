...
// ────────────────────────────────────────────────────────────────────────
const DEFAULT_GEMINI_MODEL_ID: string = "gemini-2.5-flash-lite"; 
...
const DEFAULT_MIN_API_INTERVAL_S: number = 5; 
const DEFAULT_READING_AGE_MIN: number = 5; 
const DEFAULT_READING_AGE_MAX: number = 12; 
const DEFAULT_TARGET_READING_AGE: number = 7; 

const STORY_FONT_SIZE_STEP: number = 0.1; 
const MIN_STORY_FONT_SIZE_REM: number = 0.7;
const MAX_STORY_FONT_SIZE_REM: number = 2.0;
const DEFAULT_STORY_FONT_SIZE_REM: number = 1.0; 
...
let modalApiKeyInput: HTMLInputElement | null, charactersInput: HTMLTextAreaElement | null, audienceInput: HTMLInputElement | null, craftingFrameworkSelect: HTMLSelectElement | null, frameworkSummaryDiv: HTMLElement | null, generateButton: HTMLButtonElement | null, storyTitleDiv: HTMLElement | null, storyOutputDiv: HTMLElement | null;
let settingsModal: HTMLElement | null, settingsButton: HTMLButtonElement | null, cancelSettingsButton: HTMLButtonElement | null, saveSettingsButton: HTMLButtonElement | null, modalModelSelect: HTMLSelectElement | null, downloadChatLogButton: HTMLButtonElement | null, minApiIntervalInput: HTMLInputElement | null;
let copyStoryButton: HTMLButtonElement | null, saveStoryButton: HTMLButtonElement | null, elaborateStoryButton: HTMLButtonElement | null, decreaseFontButton: HTMLButtonElement | null, increaseFontButton: HTMLButtonElement | null;
let openStoryButton: HTMLButtonElement | null, storyFileInput: HTMLInputElement | null;
let userSuggestionsTextarea: HTMLTextAreaElement | null;
let enableReadingAgeAdjustmentCheckbox: HTMLInputElement | null, targetReadingAgeSlider: HTMLInputElement | null, readingAgeSliderContainer: HTMLElement | null;
let readingAgeMinInput: HTMLInputElement | null, readingAgeMaxInput: HTMLInputElement | null;
let enableConsolidatorCheckbox: HTMLInputElement | null;
let authorStyleSelect: HTMLSelectElement | null, styleSummaryDiv: HTMLElement | null;
let toneSelect: HTMLSelectElement | null, pacingSelect: HTMLSelectElement | null, humorSelect: HTMLSelectElement | null, emotionSelect: HTMLSelectElement | null;
let agentTogglesContainer: HTMLElement | null, masterThinkingToggle: HTMLInputElement | null, agent1CrafterToggle: HTMLInputElement | null, agent2ElaboratorToggle: HTMLInputElement | null, agent3ReviewerToggle: HTMLInputElement | null, agent4PolisherToggle: HTMLInputElement | null, agent5CleanerToggle: HTMLInputElement | null, agent6TitlerToggle: HTMLInputElement | null, agentCConsolidatorToggle: HTMLInputElement | null;
let parentalControlsToggle: HTMLElement | null, parentalControlsContent: HTMLElement | null, sensitivityPresetSelect: HTMLSelectElement | null, customSensitivityControls: HTMLElement | null;
let conflictSlider: HTMLInputElement | null, scarySlider: HTMLInputElement | null, sadnessSlider: HTMLInputElement | null, complexitySlider: HTMLInputElement | null;
let conflictLabel: HTMLElement | null, scaryLabel: HTMLElement | null, sadnessLabel: HTMLElement | null, complexityLabel: HTMLElement | null, sensitivitySummary: HTMLElement | null;
let stemConceptSection: HTMLElement | null, stemConceptSelect: HTMLSelectElement | null, stemConceptHint: HTMLElement | null;
// Framework and Style modal elements
let frameworkModal: HTMLElement | null, frameworkSelectButton: HTMLButtonElement | null, frameworkSelectedLabel: HTMLElement | null, frameworkOptionsGrid: HTMLElement | null;
let styleModal: HTMLElement | null, styleSelectButton: HTMLButtonElement | null, styleSelectedLabel: HTMLElement | null, styleOptionsGrid: HTMLElement | null, closeStyleModalButton: HTMLButtonElement | null;
// Help modal elements
let helpModal: HTMLElement | null, helpButton: HTMLButtonElement | null, helpTopicsList: HTMLElement | null, helpContentDisplay: HTMLElement | null, closeHelpModalButton: HTMLButtonElement | null;
// Plot points toggle
let includePlotPointsCheckbox: HTMLInputElement | null, plotPointsContainer: HTMLElement | null;
// Theme toggle
let themeToggle: HTMLButtonElement | null;
// Assist tab and settings export controls
let assistTabButton: HTMLButtonElement | null, exportVocabularyButton: HTMLButtonElement | null, clearVocabularyButton: HTMLButtonElement | null;
let assistEmptyState: HTMLElement | null, assistWordState: HTMLElement | null, assistWordHeading: HTMLElement | null, assistDefinitions: HTMLElement | null, assistSynonyms: HTMLElement | null, assistAntonyms: HTMLElement | null, assistIpa: HTMLElement | null;
let assistSpeakButton: HTMLButtonElement | null, assistLoadingState: HTMLElement | null, assistErrorState: HTMLElement | null, assistSource: HTMLElement | null, assistLookupCount: HTMLElement | null;
let loadSampleStoryButton: HTMLButtonElement | null;
let ttsVoiceSelect: HTMLSelectElement | null, ttsGenderSelect: HTMLSelectElement | null, ttsSourceSelect: HTMLSelectElement | null;
let readAloudButton: HTMLButtonElement | null, stopReadAloudButton: HTMLButtonElement | null, readAloudLabel: HTMLElement | null;
// Phonics Assist DOM elements
let assistPhonicsSection: HTMLElement | null, phonicsChunks: HTMLElement | null, phonicsSoundOutButton: HTMLButtonElement | null, phonicsPhonemesRow: HTMLElement | null, phonicsDisclaimer: HTMLElement | null, phonicsLoadingState: HTMLElement | null;
...
let selectedStoryWordElement: HTMLElement | null = null;
let selectedAssistWord: string = "";
let selectedAssistWordNormalized: string = "";
let assistLookupRequestToken: number = 0;
let currentAssistAudioUrl: string = '';
let currentAssistAudio: HTMLAudioElement | null = null;

// Phonics Assist state
let currentPhonicsAssist: PhonicsAssist | null = null;
let isSoundingOut: boolean = false;
let soundOutAbortController: AbortController | null = null;
let currentPhonicsAudio: HTMLAudioElement | null = null;

// STEM Concept data for hints
const STEM_CONCEPT_DATA: Record<string, STEMConceptData> = {
    displacement: {
...
// Sensitivity presets
const SENSITIVITY_PRESETS: Record<string, SensitivitySettings> = {
...
let currentStoryFontSizeRem: number = DEFAULT_STORY_FONT_SIZE_REM;
...
// --- UI Helper Functions ---
function updateTargetReadingAgeSliderDOMState(): void {
...
function updateAgentTogglesUI(): void {
...
function initializeTabSystem(): void {
...
function setActiveTab(targetTab: string): boolean {
...
function setAssistTabEnabled(isEnabled: boolean): void {
...
function resetAssistPanelToEmptyState(): void {
    ...
    // Reset phonics
    if (assistPhonicsSection) assistPhonicsSection.classList.add('hidden');
    resetPhonicsSection();
...
function setAssistLoadingState(isLoading: boolean): void {
    if (!assistLoadingState) return;
    assistLoadingState.classList.toggle('hidden', !isLoading);
}

function setAssistErrorState(message: string | null): void {
...
function prepareAssistPanelForWord(word: string): void {
...
function renderAssistResult(assistData: import('./types.js').AssistData): void {
    ...
    if (assistIpa) {
        const dialectLabels: Record<string, string> = { uk: 'UK', us: 'US' };
    ...
    // Source attribution
    if (assistSource) {
        const sourceNames: Record<string, string> = { freedict: 'Free Dictionary', wiktionary: 'Wiktionary', cache: 'Cached' };
...
function renderWordChips(container: HTMLElement | null, words: string[] | undefined): void {
    if (!container) return;
    container.innerHTML = '';
...
function updateLookupCountBadge(normalizedWord: string): void {
...
function cancelAssistSpeech(): void {
    ...
    if (isReadingAloud) {
        ...
        if (readAloudButton) readAloudButton.classList.remove('hidden');
        if (stopReadAloudButton) stopReadAloudButton.classList.add('hidden');
    }
}
 ...
 */
async function loadPhonicsForWord(word: string, normalizedWord: string): Promise<void> {
    if (!assistPhonicsSection) return;
    ...
    try {
        ...
        if (normalizedWord !== selectedAssistWordNormalized) return;
        ...
        console.warn('Phonics assist unavailable:', (err as Error).message);
        // Hide phonics section gracefully if RiTa fails to load
        if (assistPhonicsSection) assistPhonicsSection.classList.add('hidden');
    } finally {
        if (phonicsLoadingState) phonicsLoadingState.classList.add('hidden');
    }
}

function resetPhonicsSection(): void {
    currentPhonicsAssist = null;
    stopSoundOut();
    if (phonicsChunks) phonicsChunks.innerHTML = '';
...
function renderPhonicsSection(assist: PhonicsAssist): void {
    if (!assistPhonicsSection || !phonicsChunks) return;
...
function renderPhonemeOnlyChips(assist: PhonicsAssist): void {
...
function handlePhonicsChunkClick(index: number): void {
    if (!currentPhonicsAssist || !currentPhonicsAssist.chunks[index]) return;
    const chunk = currentPhonicsAssist.chunks[index];
    if (!chunk.phoneme && !chunk.ttsHint) return;
...
function speakPhonemeOnly(phoneme: string, index: number): void {
    cancelAssistSpeech();
    ...
    if (phonicsPhonemesRow) {
        ...
        if (chip) chip.classList.add('is-highlighted');
    }
...
const phonemeAudioCache: Record<string, boolean | undefined> = {};
 ...
 */
function phonemeAudioPath(phoneme: string): string | null {
 ...
 */
function tryPlayPhonemeAudio(phoneme: string): Promise<boolean> {
    return new Promise((resolve) => {
        if (!phoneme) { resolve(false); return; }

        // Already known to be missing
        if (phonemeAudioCache[phoneme] === false) { resolve(false); return; }
 ...
 */
async function speakPhonicsSound(phoneme: string, ttsHint: string, onEnd?: () => void): Promise<void> {
 ...
 */
function speakPhonicsHintTTS(text: string, onEnd?: () => void): void {
...
function highlightPhonicsChunk(index: number): void {
    if (!phonicsChunks) return;
...
function clearPhonicsHighlight(): void {
    if (phonicsChunks) {
        phonicsChunks.querySelectorAll('.phonics-chunk').forEach(el => el.classList.remove('is-highlighted'));
    }
    if (phonicsPhonemesRow) {
 ...
 */
async function startSoundOut(): Promise<void> {
    if (!currentPhonicsAssist) return;
...
function stopSoundOut(): void {
    ...
    if (currentPhonicsAudio) {
        try { currentPhonicsAudio.pause(); currentPhonicsAudio.currentTime = 0; } catch (_) {}
        currentPhonicsAudio = null;
    }
    if ('speechSynthesis' in window) {
        window.speechSynthesis.cancel();
...
function handleSoundOutClick(): void {
    if (isSoundingOut) {
        stopSoundOut();
    } else {
        startSoundOut();
...
function getSelectedTTSVoice(): SpeechSynthesisVoice | null {
...
function classifyVoiceGender(voice: SpeechSynthesisVoice): string {
...
function compareVoicesForStability(a: SpeechSynthesisVoice, b: SpeechSynthesisVoice): number {
...
function populateTTSVoiceDropdown(): void {
...
function updateBrowserVoiceSettingsVisibility(): void {
...
let ttsPrimed: boolean = false;
function primeTTSAudio(): void {
...
function getTTSSource(): string {
    return loadFromLocalStorage(LS_TTS_SOURCE) || 'browser';
}

function speakSelectedAssistWord(): void {
...
function resetAssistSelectionForNewStory(): void {
...
let isReadingAloud: boolean = false;
let readAloudQueue: SpeechSynthesisUtterance[] = [];
 ...
 */
function getReadAloudText(): string {
    ...
    if (selectedStoryWordElement && words.length > 0) {
        ...
        if (idx >= 0) startNode = selectedStoryWordElement;
    }
    ...
    try {
        ...
        if (blocks.length === 0) return storyOutputDiv.textContent?.trim() || '';
        ...
        for (let i = 0; i < blocks.length; i++) {
            ...
            if (block.classList.contains('story-break')) continue;
...
function startReadAloud(): void {
    ...
    for (let i = 0; i < chunks.length; i++) {
        ...
        if (i === 0) {
            utterance.onstart = () => {
                isReadingAloud = true;
                if (readAloudButton) readAloudButton.classList.add('hidden');
                if (stopReadAloudButton) stopReadAloudButton.classList.remove('hidden');
            };
        }
...
function updateReadAloudButtonLabel(): void {
    if (!readAloudLabel) return;
...
async function lookupAssistDataForWord(selectedWord: string, normalizedWord: string): Promise<void> {
...
function handleStoryWordClick(event: Event): void {
    const wordElement = (event.target as HTMLElement).closest('.story-word') as HTMLElement | null;
...
function handleAssistChipClick(event: Event): void {
    const chip = (event.target as HTMLElement).closest('.assist-word-chip') as HTMLElement | null;
    if (!chip) return;
...
function populateFrameworkModal(): void {
...
function populateStyleModal(): void {
...
function populateHelpTopics(): void {
...
function selectHelpTopic(topicKey: string): void {
    ...
    helpTopicsList.querySelectorAll('.help-topic-btn').forEach(btn => {
        btn.classList.toggle('active', (btn as HTMLElement).dataset.topic === topicKey);
    });
...
function updateSTEMSectionVisibility(): void {
...
function updateSTEMConceptHint(conceptKey: string): void {
 ...
 */
function getSelectedSTEMConcept(): SelectedSTEMConcept | null {
 ...
 */
function getSelectedPersonaText(): string {
    return "";
}

// --- Parental Controls Functions ---
function updateSensitivitySliderLabels(): void {
...
function applySensitivityPreset(presetKey: string): void {
    const preset = SENSITIVITY_PRESETS[presetKey];
    if (!preset) return;
    
    if (conflictSlider) conflictSlider.value = String(preset.conflict);
    if (scarySlider) scarySlider.value = String(preset.scary);
    if (sadnessSlider) sadnessSlider.value = String(preset.sadness);
    if (complexitySlider) complexitySlider.value = String(preset.complexity);
...
function saveSensitivitySettings(): void {
...
function loadSensitivitySettings(): void {
...
function updateSensitivitySummary(): void {
 ...
 */
function getCurrentSensitivitySettings(): SensitivitySettings | null {
    const preset = sensitivityPresetSelect ? sensitivityPresetSelect.value : 'standard';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Get DOM elements
    modalApiKeyInput = $id<HTMLInputElement>('modalApiKeyInput');
    modalModelSelect = $id<HTMLSelectElement>('modalModelSelect');
    minApiIntervalInput = $id<HTMLInputElement>('minApiIntervalInput');
    readingAgeMinInput = $id<HTMLInputElement>('readingAgeMinInput');
    readingAgeMaxInput = $id<HTMLInputElement>('readingAgeMaxInput');
    downloadChatLogButton = $id<HTMLButtonElement>('downloadChatLogButton');
    exportVocabularyButton = $id<HTMLButtonElement>('exportVocabularyButton');
    clearVocabularyButton = $id<HTMLButtonElement>('clearVocabularyButton');
    charactersInput = $id<HTMLTextAreaElement>('charactersInput');
    audienceInput = $id<HTMLInputElement>('audienceInput');
    userSuggestionsTextarea = $id<HTMLTextAreaElement>('userSuggestionsTextarea');
    enableReadingAgeAdjustmentCheckbox = $id<HTMLInputElement>('enableReadingAgeAdjustmentCheckbox');
    targetReadingAgeSlider = $id<HTMLInputElement>('targetReadingAgeSlider');
    readingAgeSliderContainer = $id('readingAgeSliderContainer');
    enableConsolidatorCheckbox = $id<HTMLInputElement>('enableConsolidatorCheckbox');
    craftingFrameworkSelect = $id<HTMLSelectElement>('craftingFrameworkSelect');
    frameworkSummaryDiv = $id('frameworkSummary');
    generateButton = $id<HTMLButtonElement>('generateButton');
    storyTitleDiv = $id('storyTitle');
    storyOutputDiv = $id('storyOutput');
    copyStoryButton = $id<HTMLButtonElement>('copyStoryButton');
    saveStoryButton = $id<HTMLButtonElement>('saveStoryButton');
    openStoryButton = $id<HTMLButtonElement>('openStoryButton');
    storyFileInput = $id<HTMLInputElement>('storyFileInput');
    elaborateStoryButton = $id<HTMLButtonElement>('elaborateStoryButton');
    readAloudButton = $id<HTMLButtonElement>('readAloudButton');
    readAloudLabel = $id('readAloudLabel');
    stopReadAloudButton = $id<HTMLButtonElement>('stopReadAloudButton');
    decreaseFontButton = $id<HTMLButtonElement>('decreaseFontButton');
    increaseFontButton = $id<HTMLButtonElement>('increaseFontButton');
    settingsModal = $id('settingsModal');
    settingsButton = $id<HTMLButtonElement>('settingsButton');
    cancelSettingsButton = $id<HTMLButtonElement>('cancelSettingsButton');
    saveSettingsButton = $id<HTMLButtonElement>('saveSettingsButton');
    authorStyleSelect = $id<HTMLSelectElement>('authorStyleSelect');
    styleSummaryDiv = $id('styleSummary');
    toneSelect = $id<HTMLSelectElement>('toneSelect');
    pacingSelect = $id<HTMLSelectElement>('pacingSelect');
    humorSelect = $id<HTMLSelectElement>('humorSelect');
    emotionSelect = $id<HTMLSelectElement>('emotionSelect');
    agentTogglesContainer = $id('agentTogglesContainer');
    masterThinkingToggle = $id<HTMLInputElement>('masterThinkingToggle');
    agent1CrafterToggle = $id<HTMLInputElement>('agent1CrafterToggle');
    agent2ElaboratorToggle = $id<HTMLInputElement>('agent2ElaboratorToggle');
    agent3ReviewerToggle = $id<HTMLInputElement>('agent3ReviewerToggle');
    agent4PolisherToggle = $id<HTMLInputElement>('agent4PolisherToggle');
    agent5CleanerToggle = $id<HTMLInputElement>('agent5CleanerToggle');
    agent6TitlerToggle = $id<HTMLInputElement>('agent6TitlerToggle');
    agentCConsolidatorToggle = $id<HTMLInputElement>('agentCConsolidatorToggle');
    stemConceptSection = $id('stemConceptSection');
    stemConceptSelect = $id<HTMLSelectElement>('stemConceptSelect');
    ...
    sensitivityPresetSelect = $id<HTMLSelectElement>('sensitivityPresetSelect');
    customSensitivityControls = $id('customSensitivityControls');
    conflictSlider = $id<HTMLInputElement>('conflictSlider');
    scarySlider = $id<HTMLInputElement>('scarySlider');
    sadnessSlider = $id<HTMLInputElement>('sadnessSlider');
    complexitySlider = $id<HTMLInputElement>('complexitySlider');
    ...
    // Assist tab elements
    assistTabButton = $id<HTMLButtonElement>('assistTabButton');
    ...
    assistSpeakButton = $id<HTMLButtonElement>('assistSpeakButton');
    ...
    loadSampleStoryButton = $id<HTMLButtonElement>('loadSampleStoryButton');
    ttsVoiceSelect = $id<HTMLSelectElement>('ttsVoiceSelect');
    ttsGenderSelect = $id<HTMLSelectElement>('ttsGenderSelect');
    ttsSourceSelect = $id<HTMLSelectElement>('ttsSourceSelect');

    // Phonics Assist DOM elements
    assistPhonicsSection = $id('assistPhonicsSection');
    phonicsChunks = $id('phonicsChunks');
    phonicsSoundOutButton = $id<HTMLButtonElement>('phonicsSoundOutButton');
    ...
    frameworkSelectButton = $id<HTMLButtonElement>('frameworkSelectButton');
    frameworkSelectedLabel = $id('frameworkSelectedLabel');
    frameworkOptionsGrid = $id('frameworkOptionsGrid');
    styleModal = $id('styleModal');
    styleSelectButton = $id<HTMLButtonElement>('styleSelectButton');
    styleSelectedLabel = $id('styleSelectedLabel');
    styleOptionsGrid = $id('styleOptionsGrid');
    closeStyleModalButton = $id<HTMLButtonElement>('closeStyleModalButton');
    
    // Plot points toggle
    includePlotPointsCheckbox = $id<HTMLInputElement>('includePlotPointsCheckbox');
    plotPointsContainer = $id('plotPointsContainer');
    
    // Help modal elements
    helpModal = $id('helpModal');
    helpButton = $id<HTMLButtonElement>('helpButton');
    helpTopicsList = $id('helpTopicsList');
    helpContentDisplay = $id('helpContentDisplay');
    closeHelpModalButton = $id<HTMLButtonElement>('closeHelpModalButton');
...
- End with a clear understanding of the concept that the child reader can take away`;
        ...
        if (!canThink || !masterThinkingEnabled) {
            Object.keys(agentThinkingConfig).forEach(key => (agentThinkingConfig as Record<string, boolean>)[key] = false);
        }
    ...
    async function handleGenerateStory() {
        ...
        try {
            ...
            displayErrorInStoryOutput((error as Error).message || "An unknown error occurred during story generation.");
            setAssistTabEnabled(false);
        } finally {
            enableMainControls(); 
        }
    ...
    async function handleElaborateStory() {
        ...
        try {
            ...
            displayErrorInStoryOutput((error as Error).message || "An unknown error occurred during story elaboration.");
            setAssistTabEnabled(false);
        } finally {
            enableMainControls();
        }
