// src/prompts/adjustment_modules.ts
...
export const ADJUSTMENT_MODULES: Record<string, Record<string, string>> = {
    tone: {
        'none': '',
...
const SENSITIVITY_GUIDANCE_TEMPLATES: Record<string, Record<number, string>> = {
    conflict: {
 ...
 */
export function getSensitivityGuidance(settings: { conflict?: number; scary?: number; sadness?: number; complexity?: number } | null): string {
    if (!settings) return '';
