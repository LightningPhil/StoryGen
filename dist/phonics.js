 ...
 */
export function alignGraphemesToPhones(word, phonesUpper) {
    const w = word.toLowerCase();
    const phones = phonesUpper.map(normalizePhone);
    function dfs(iChar, iPh) {
        if (iChar === w.length && iPh === phones.length)
            return [];
        if (iChar >= w.length || iPh > phones.length)
            return null;
        for (const entry of SORTED_GRAPHEMES) {
            const { g, p } = entry;
            if (!w.startsWith(g, iChar))
                continue;
            // Check phoneme sequence match
            if (iPh + p.length > phones.length)
                continue;
            let ok = true;
 ...
 */
export function ttsHintForPhoneme(p) {
    if (!p)
        return '';
    if (TTS_HINT_MAP[p] !== undefined)
        return TTS_HINT_MAP[p];
 ...
 */
export function buildPhonicsAssist(word) {
