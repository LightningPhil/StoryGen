...
export async function callAgentAPI(prompt, currentApiKey, selectedModelId, agentName = "Agent", retryAttempt = 0, currentRunChatLogArray, storyOutputDivRef, minApiIntervalMs, enableThinking = false, responseMimeType = '') {
    if (!currentApiKey) {
    ...
    if (retryAttempt === 0) { // Only apply initial wait if not a retry
        ...
        if (timeSinceLastFetchInitiated < minApiIntervalMs) {
            ...
            if (waitTime > 0) {
                ...
                console.log(waitMsg);
                if (storyOutputDivRef && storyOutputDivRef.textContent !== undefined)
                    storyOutputDivRef.textContent += waitMsg;
    ...
    try {
        ...
        if (response.status === 503 || response.status === 429) {
            if (retryAttempt < MAX_RETRIES) {
                ...
                if (storyOutputDivRef && storyOutputDivRef.textContent !== undefined)
                    storyOutputDivRef.textContent += retryMsg;
