...
export function initUIElements(elements) {
    ...
    if (saveStoryButton)
        saveStoryButton.classList.add('hidden');
    if (elaborateStoryButton)
        elaborateStoryButton.classList.add('hidden');
    if (decreaseFontButton)
        decreaseFontButton.classList.add('hidden');
    if (increaseFontButton)
        increaseFontButton.classList.add('hidden');
    if (storyTitleDiv) {
...
export function displayFinalStoryOutput(title, storyText, isElaboration = false) {
    ...
    if (saveStoryButton)
        saveStoryButton.classList.remove('hidden');
    if (decreaseFontButton)
        decreaseFontButton.classList.remove('hidden');
    if (increaseFontButton)
        increaseFontButton.classList.remove('hidden');
    if (elaborateStoryButton) {
...
export function updateAuthorStyleSummaryDisplay(STORY_STYLE_SUMMARIES_DATA) {
    if (!authorStyleSelect || !styleSummaryDiv)
        return;
...
export function populateDropdown(selectElement, optionsObject, capitalize = true) {
    if (!selectElement || !optionsObject)
        return;
...
export function formatStoryAsHtml(text) {
    ...
    while (i < lines.length) {
        ...
        while (i < lines.length) {
            ...
            if (/^(\*[\s*]*\*[\s*]*\*|---+|___+)$/.test(pt))
                break;
            if (/^\x00CODEBLOCK_\d+\x00$/.test(pt))
                break;
            paraLines.push(pt);
            i++;
        }
...
function renderTable(tableLines) {
    ...
    let alignments = headerCells.map(() => '');
    let bodyStart = 1;
